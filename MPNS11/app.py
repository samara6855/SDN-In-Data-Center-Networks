...
def nreq(id):
    cursor = mydb.cursor()
    cursor.execute('SELECT fid FROM kreq where id ='+id)
    account = cursor.fetchone()
...
def ustore():
    if request.method == 'POST':
        ...
        n = f.filename
        fpath = "E:/MiniProject/MPNS11/static/file/"+n
        f.save(fpath)
        ...
        efname = "E:/MiniProject/MPNS11/static/file/enc/"+n
        with open(efname, 'wb') as encrypted_file:
            encrypted_file.write(encrypted)
        decrypted = fernet.decrypt(encrypted)
        dfname = "E:/MiniProject/MPNS11/static/file/dec/"+n
...
def ureq(id):
    ...
    account = cursor.fetchone()
    var = (account[0], uid, 'pending', 'pending')
    cursor.execute('insert into kreq values (0, %s, %s, %s, %s)', var)
    mydb.commit()
    cursor.close()
    if cursor.rowcount == 1:
        return render_template('uhome.html', result = session['name'])
